...
3. **Centralized Pooled-Data Baseline**:
   ...
   - Use the **exact same assembled `MultimodalNet` architecture** (incorporating Member 2's ViT-based `ImageEncoder` & Fusion module + your `ClinicalEncoder`).
...
4. **Clinical Evaluation Metrics Suite**:
   ...
   - *Note on Downstream Explainability (Grad-CAM)*: While formal explainability is planned post-30%, note that downstream Grad-CAM analysis will directly utilize the image-token representations generated by Member 2's Vision Transformer (ViT) to identify contributing radiographic lung regions.
...
- **Receives from Member 2**: `ImageEncoder` (ViT-based) and `FusionClassifier` modules to assemble `MultimodalNet`.
